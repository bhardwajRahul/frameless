...
type Migrator[R resourceConnection] struct {
	// Connection is the ResourceConnection where the migration steps needs to operate.
	Connection R
...
func (m Migrator[R]) Migrate(ctx context.Context) (rErr error) {
	...
	for _, step := range m.Steps.stages() {
		if err := m.up(schemaCTX, stepCTX, step.Version, step.Step); err != nil {
			return err
		}
	}
